...
public class ResponseTest02 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*
		 * - redirect방식 ==> 다른 웹페이지로 넘어가도록한다. (직접 파라미터를 넘길 수 없다.)
		 ...
		 *                   (이유: 브라우저에게 새롭게 요청하기 때문이다.)    
		 *                    
		 * - 이동할 때 사용하는 메서드 : response객체의 sendRedirect()메서드이다. 
		 *                  형식) sendRedirect("URL주소"); 
		 *                  URL주소 ==> 'Context Path' 부터 기술한다.
		 *                  예) 이동할 주소가 "http://localhost/testWeb/test.do"라고 하면 
	     *                      컨텍스트패스 부터의 주소인 "/testWeb/test.do"와 같이 기술하면 된다.   
		 */
		
		// 이동할 페이지 URL주소 : responseRedirectTest.do
		// ==> http://localhost/testWeb/reponseRedirectTest.do
		
		//request.setAttribute("tel", "010-1234-5678"); // 사용불가
