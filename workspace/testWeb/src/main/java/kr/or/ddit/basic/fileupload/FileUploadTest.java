...
/*
	- 서블릿 버전 3.0이상에서 파일 업로드를 하려면 서블릿에 @MultipartConfig
	  애노테이션을 설정해야 한다.
...
public class FileUploadTest extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 파일이 아닌 일반 파라미터 데이터는 getParametr()메서드를
		// 이용해서 값을 구한다.
		...
		1) request.getParts(); 
			==> 전체 Part객체가 저장된 컬렉션이 반환된다.
		2) request.getPart("part이름");
			==> 지정한 이름에 맞는 Part객체를 반환한다.	
		 */
		// 전체 Part객체 개수만큼 반복을 한다.
