	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 - Cookie 저장하는 방법
		 	...
		 	==> 한 도메인당 20개, 총 300개까지 저장가능하다.
		 	
		 1) Cookie객체를 생성한다. ==> '쿠키변수'와 '쿠키값'은 문자열로 지정한다.
		 형식) Cookie cookie변수 = new Cookie("쿠키변수", "쿠키값");
		 	==> 쿠키값으로 한글을 사용할 경우에는 URLEncoder.encode()메서드 
		 		인코딩한 후 저장한다.
		 */
		Cookie nameCookie = 
				new Cookie("name", URLEncoder.encode("홍길동", "utf-8"));
		Cookie ageCookie =
				new Cookie("age", String.valueOf(30));
		Cookie genderCookie = 
				new Cookie("gender", "Male");
		...
		/*
		 2) 쿠키의 속성을 설정한다.
		 	...
		 	- 쿠키변수.setMaxAge(유지시간); ==> 단위(초)
		 		==> 유지시간 -1 : 브라우저가 종료될 때까지 유지된다.(기본값)
		 		==> 0 : 즉시 쿠키가 삭제된다.
		 	- 쿠키변수.setDomain("적용도메인명");
		 		==> 예) ".ddit.or.kr" 
		 				--> www.ddit.or.kr, www.mail.ddit.or.kr
		 	- 쿠키변수.setSecure(보안여부); 
