...
public class CryptoUtil {
	...
	public static String sha512(String str) throws NoSuchAlgorithmException, UnsupportedEncodingException {
		...
		// 알고림즘 이름 종류 : MD5, SHA-256, SHA-512 등...
...
//				MessageDigest.getInstance("MD5");
		...
		// md.digest() --> 암호화를 시키는 명령어 바이트 배열로 넘어와서 인코딩을 시켜서 문자열로 한다.
	...
	// 암호화를 하는 메서드와 암호화를 푸는 메서드가 필요하다.
	...
	public static String encryptoAES256(String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		/* 256방식은 원본데이터를 128bit씩 자른다. 자른 것들은 하나의 블럭으로 본다.
		 바로 암호화를 하는 것이 아니라 값을 가지고, XOR연산을 해서 키값과 연결해서 암호화한다.
		 암호화한 결과가 나온다. 그 다음 블럭으로 계싼을 한다. XOR연산에 필요한 값을 첫번째 블럭의
		 결과값으로 연산을 한다.
		 처음에 값을 iv라고 한다. 
		*/
		...
		*/ 
		...
		/*
		 ...
		 - CBC : Cipher Block Chaing Mode ==> 동일한 평문 블록과
		 	     암호문 블록의 쌍이 발생하지 않도록 이전 단계의 암복호화한
